...
Satellite::Satellite(std::string name, int diameter, float distance, float inclination, Planet *planet, CustomProgram *program)
  ...
  this->m_sphere = new Sphere(/*(this->m_diameter/2) * distRatio*/ 0.2, 32, 16);
  this->m_pos = glm::vec3(this->m_distance, 0, 0);
...
void Satellite::move() {
  this->m_modelMatrix = glm::rotate(this->m_planet->getModelMatrix(), glm::radians(this->m_inclination), glm::vec3(0, 0, 1));
  this->m_modelMatrix = glm::rotate(this->m_modelMatrix, App::instance()->getTime(), glm::vec3(0, 1, 0));
